...
def cost_stress(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for bps in [0, 20, 50, 100]:
        ...
        rows.append({"cost_bps": bps, **metric(returns), "avg_turnover": frame["turnover"].mean()})
    return pd.DataFrame(rows)


def block_bootstrap(frame: pd.DataFrame, simulations: int = 5_000, block: int = 6) -> pd.DataFrame:
    excess = (frame["net_return"] - frame["benchmark_return"]).dropna().to_numpy()
    ...
    return pd.DataFrame([{
        "block_months": block, "simulations": simulations, "annual_excess_estimate": excess.mean() * 12,
        "ci_2.5%": low, "median": median, "ci_97.5%": high,
